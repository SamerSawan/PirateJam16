...
## Set target pos safely, ensuring that it navigates even when the target is outside the nav area
func set_target_position_safely(target_pos : Vector2, origin : Vector2 = root.global_position, rid: RID = Utils.DEFAULT_RID): # sets target to closest point that is within the default rid navigation map
	if synced:
		# find navigation tile that has (matching x, closest y) to target position
		closest_point = NavigationServer2D.map_get_closest_point(rid, target_pos)
		...
		if closest_point:
				...
				Debug.spawn_debug_cross_shape(closest_point)
				return true
...
#func _check_path(origin : Vector2, destination : Vector2, nav_layers : int = 1, optimize : bool = true, rid : RID = Utils.DEFAULT_RID):
	#var path = NavigationServer2D.map_get_path(rid, origin, destination, optimize)
...
#func _is_path_reachable(path):
	...
	#return true
#
#func _calculate_alternate_path(origin : Vector2, destination : Vector2, _nav_layers : int = 1, optimize : bool = true, rid : RID = Utils.DEFAULT_RID):
